DROP FUNCTION IF EXISTS public.search_passages(text, text, integer);
...
judged AS (
  SELECT c.sid, c.pid, c.loc, c.txt, c.stitle, c.surl,
         ...
         CASE
           ...
           WHEN NOT (fu.fts_raw >= pr.fts_bar OR fu.trgm_raw >= pr.trgm_bar OR fu.anchor_raw > 0)
             THEN 'Matched weakly; below the relevance bar (keyword ' || fu.fts_raw::text ||
                  ' < ' || pr.fts_bar::text || ', wording similarity ' || fu.trgm_raw::text ||
                  ' < ' || pr.trgm_bar::text || ', no citation-anchor match).'
           ELSE NULL
         END AS reason
  FROM fused fu
